...
当前兼容性目标是 Windows 和 macOS。仓库采用单一 Skill、共享业务 reference、平台专属执行 reference 的结构，不复制两套容易漂移的业务规则：
...
Codex 默认安装目录为 `${CODEX_HOME:-~/.codex}/skills`。Windows PowerShell：

```powershell
git clone https://github.com/ZyoHuang/siyuan-notes "$HOME\.codex\skills\siyuan-notes"
```

macOS：

```bash
git clone https://github.com/ZyoHuang/siyuan-notes "${CODEX_HOME:-$HOME/.codex}/skills/siyuan-notes"
```
...
完整命令见 `references/platform-windows.md` 和 `references/platform-macos.md`。
