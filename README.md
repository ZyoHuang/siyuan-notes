...
本 skill 采用渐进加载结构：核心工作流位于 `SKILL.md`，详细 CLI/API 资料放在 `references/`，仅在相关任务触发时读取。
...
如果运行时不支持自动加载 skill，可以把 `SKILL.md` 和当前任务涉及的 `references/*.md` 一起提供给 Agent。仅粘贴 `SKILL.md` 不包含完整 API 参考。

## 目录结构 Layout

- `SKILL.md` — skill 入口、核心工作流和安全边界。
- `references/` — 按需读取的概念、CLI、内核 API、数据库 API 与 Inbox 参考。
