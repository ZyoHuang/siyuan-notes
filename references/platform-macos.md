...
本文件只适用于 macOS。使用 POSIX shell 语法调用 SiYuan CLI；不要把 PowerShell 语法混入命令。

## CLI 发现顺序

先查 `PATH`，失败后再检查标准应用包内的官方二进制：

```bash
SIYUAN_BIN="$(command -v siyuan 2>/dev/null || true)"

if [ -z "$SIYUAN_BIN" ]; then
  candidate="/Applications/SiYuan.app/Contents/Resources/kernel/SiYuan-Kernel"
  if [ -x "$candidate" ]; then
    SIYUAN_BIN="$candidate"
  fi
fi

if [ -z "$SIYUAN_BIN" ]; then
  printf '%s\n' '未找到 SiYuan CLI。请确认应用位于 /Applications，并参考下方官方符号链接方式。' >&2
  exit 1
fi
...
```bash
export SIYUAN_WORKSPACE_PATH="/Users/you/SiYuan"
...
SiYuan 官方 CLI 指南要求 macOS 安装后创建符号链接：
