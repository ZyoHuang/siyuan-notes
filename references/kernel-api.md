...
官方文档跟踪 `master`，因此把本章节视为路由指南而非不可变 schema。当实时内核版本不同、端点返回不同结构或字段被弃用时，重新打开官方来源。Inbox 路由不在公开 API 文档中；单独使用 [inbox.md](inbox.md)。
...
- 数据库 `avID`、嵌入 `blockID`、视图 `viewID`、行 `itemID`、字段 `keyID`：各自不同；见 [database-api.md](database-api.md)。
...
其余 16 个官方 `/api/av/*` 路由编录于 [database-api.md](database-api.md)。任何数据库请求前先阅读该文件。
...
- 数据库：重新渲染确切视图并比较行、字段、filter、sort 或布局状态；见 [database-api.md](database-api.md)。
