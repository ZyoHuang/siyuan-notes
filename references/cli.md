...
3.7.1 CLI 已提供 `inbox list`、`inbox get` 和 `inbox convert`。处理 `Inbox` 或 `收集箱` 请求前先阅读 [inbox.md](inbox.md)。Inbox 仍是云端速记，不属于本地文档搜索或 SQL 索引。

对于 CLI 缺失或在 HTTP 上结构更丰富的官方内核能力，阅读 [kernel-api.md](kernel-api.md)。对于 `/api/av/*` 数据库操作，同时阅读 [database-api.md](database-api.md)。当两个入口都支持同一操作时优先用 CLI，因为实时帮助和 dry-run 让目标校验更安全。
