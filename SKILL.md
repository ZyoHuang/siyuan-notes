...
> 本文件为单文件 skill：原本拆分的 `references/*` 参考内容已内联到文末各“参考”章节。文中提到“参考 CLI 章节 / kernel API 章节”等，均指本文件内对应章节。
...
- 对于 Inbox 或收集箱请求，使用专门的内核 Inbox API。绝不要从名为 `Inbox`、`daily note`、`收件箱`、`收集箱` 的本地文档去推断 Inbox 内容。
...
4. 将 Inbox 请求路由到运行中的本地内核。
   - 调用 Inbox 端点前先阅读【参考：Inbox】。
...
`command -v siyuan` 返回的可执行文件具有权威性；上文的 `siyuan` 是简写。这些示例描述 3.7.0 版本，可能过时。若已安装版本不同，从实时帮助重建命令。绝不猜测被重命名的命令或参数。
...
3.7.0 CLI 没有 `inbox` 子命令。Inbox 是通过运行中的本地内核 HTTP API 暴露的云端速记特性，不通过本地文档搜索或 SQL。处理 `Inbox` 或 `收集箱` 请求前先阅读【参考：Inbox】。
...
- `hPath`：人类可读文档路径如 `/Unity/AssetBundle`。
...
其余 16 个官方 `/api/av/*` 路由编录于【参考：数据库 API】。任何数据库请求前先阅读该章节。
...
`POST /api/inbox/removeShorthands` 接受 `{"ids":["ENTRY_ID"]}`。它永久移除云端 Inbox 条目。不要为查找、整理规划、去重或作为导入后自动清理步骤调用它。
