...
本文件是轻量入口。先根据任务读取必要的 `references/*.md`，不要默认读取全部参考文件。除非用户明确要求把内容复制进个人笔记，否则将仓库文档、源代码文件和生成的交付物与个人笔记分开。
...
1. 运行 `command -v siyuan`，使用返回的可执行文件绝对路径。
2. 每次触发工作流都运行 `<binary> --version`。
3. 构造命令前运行 `<binary> --help`，再运行实际会用到的 `<binary> <command> --help` 和必要的嵌套子命令帮助。
4. 把 reference 中的命令示例视为历史参考。已安装 CLI 帮助、实时内核响应和官方文档具有更高优先级。
5. 若 CLI、内核版本或响应结构和 reference 不一致，先按实时帮助重建最小命令；仍不确定时停止并说明差异。
6. 不要安装或升级 SiYuan，不要修改 shell 或全局包配置。
...
- CLI/HTTP 返回 JSON 时，优先投影必要字段，例如 `id`、`rootID`、`hPath`、`type`、`subType`、`markdown`、`ial.updated`。
